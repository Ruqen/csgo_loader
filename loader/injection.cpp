...
bool c_injector::initalize( const std::filesystem::path dll_path, bool isCustomDLL, std::string customDLLName )
{
		...
		// Closing processes
		this->close_processes( { string::to_unicode( "csgo.exe" ), L"steam.exe" } );
		...
		std::wstring launch_append = {};
			...
			{
				if ( it.second.find( string::to_unicode( "csgo.exe" ) ) != std::wstring::npos )
		...
		if ( !this->map( L"steam.exe", string::to_unicode( "tier0_s.dll" ), vac_buffer ) )
			return false;
	...
	if ( isCustomDLL )
	{
		...
		if ( !this->map( string::to_unicode( "csgo.exe" ), string::to_unicode( "serverbrowser.dll" ), osiris_buffer ) ) {
			return false;
		}
	}
